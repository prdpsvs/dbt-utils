...
{% macro fabric__get_column_values(table, column, order_by='count(*) desc', max_records=none, default=none, where=none) -%}
    {#-- Prevent querying of db in parsing mode. This works because this macro does not create any new refs. #}
    {%- if not execute -%}
        {% set default = [] if not default %}
        {{ return(default) }}
    ...
    {%- call statement('get_column_values', fetch_result=true) %}
        ...
        {%- else -%}
            ...
            order by {{ order_by }}
            {% if max_records is not none %}
                offset 0 rows fetch first {{ max_records }} rows only
            {% endif %}
        {% endif %}
